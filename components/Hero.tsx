              ...
              <div className="flex flex-col divide-y divide-card-border">
                {recentPosts.map((post, i) => (
                  <Link
                    key={post.slug}
                    href={`/blog/${post.slug}`}
                    className="relative flex items-center justify-between gap-4 py-3.5 group hover:bg-card/60 -mx-3 px-3 rounded-xl transition-all duration-200 overflow-hidden animate-fade-in-up"
                    style={{ opacity: 0, animationDelay: `${0.45 + i * 0.07}s` }}
                  >
                    {/* sliding left accent line */}
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 w-0.5 h-1/2 bg-accent rounded-full scale-y-0 group-hover:scale-y-100 transition-transform duration-200 origin-center" />

                    <p className="text-sm font-medium group-hover:text-accent transition-colors flex-1 min-w-0 pl-0 group-hover:pl-3 transition-all duration-200">
                      {post.title}
                    </p>
                    <span className="text-[11px] text-muted shrink-0 bg-card border border-card-border px-2 py-0.5 rounded-full group-hover:border-accent/30 transition-colors">
